...
namespace Meangpu.UI
{
    ...
    {
        // Normal grid layout don't have adaptive size option
        // It add adaptive size to vertical and horizontal
        ...
        public override void CalculateLayoutInputVertical()
        {
            ...
            switch (alignment)
            {
                case Alignment.Horizontal:
                    cellWidth = (rectTransform.rect.width / (float)columns) - ((spacing.x / (float)columns) * (columns - 1)) - (padding.left / (float)columns) - (padding.right / (float)columns);
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                switch (m_ChildAlignment)
                {
                    ...
                    case TextAnchor.UpperCenter:
                        xPos += 0.5f * (gameObject.GetComponent<RectTransform>().sizeDelta.x + (spacing.x + padding.left + padding.left) - (columns * (cellSize.x + spacing.x + padding.left))); //Center xPos
                                                                                                                                                                                                 //No need to change yPos;
                        break;

                    case TextAnchor.UpperRight:
                        xPos = -xPos + gameObject.GetComponent<RectTransform>().sizeDelta.x - cellSize.x; //Flip xPos to go bottom-up
                                                                                                          //No need to change yPos;
                        break;
