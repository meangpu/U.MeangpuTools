    ...
    public static class VFavorites
    {
        static void WrappedOnGUI(object _)
        {
                ...
                void page(Rect pageRect, Page page)
                {
                    void findSelectedItem()
                    {
                        ...
                        foreach (var item in page.items)
                        {
                            ...
                            if (item.isFolder)
                            {
                                var targetBrowser = isWrappedBrowserLocked ? allBrowsers.FirstOrDefault(r => !r.GetMemberValue<bool>("isLocked")) : wrappedBrowser;
            ...
            void widget()
            {
                ...
                void cancelRenameButton()
                {
                    ...
                    var activated = (curEvent.isMouseUp && buttonRect.IsHovered()) || curEvent.keyCode == KeyCode.Escape;
                ...
                void acceptRenameButton()
                {
                    ...
                    var activated = (curEvent.isMouseUp && buttonRect.IsHovered()) || curEvent.keyCode == KeyCode.Return;
            ...
            void keys()
            {
                ...
                {
                    if (!curEvent.isKeyDown) return;
                    if (curEvent.keyCode != KeyCode.L) return;
                ...
                void unlock()
                {
                    ...
                    if (curEvent.keyCode != KeyCode.L) return;
                    if (!lockedBrowser) return;
                    if (EditorWindow.mouseOverWindow != wrappedBrowser) return;
        ...
        static void SelectItem(Item item)
        {
            ...
            void openFolder_locked()
            {
                ...
                if (!unlockedBrowser) return;

                openFolder(unlockedBrowser, item.assetPath);
        ...
        static EditorWindow lockedBrowser
        {
            get
            {
                ...
                _lockedBrowser = Resources.InstanceIDToObject(lockedBrowserInstanceId) as EditorWindow;
