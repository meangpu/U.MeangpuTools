    ...
    public static class VTabs
    {
        static void Update()
        {
            ...
            void scrollInteractions()
            {
                ...
                void sidescroll()
                {
                    ...
                    if (sidesscrollPosition.RoundToInt() != (sidesscrollPosition += sidescrollDelta).RoundToInt())
                        if (lastEvent.control || lastEvent.command)
                            moveTab(lastEvent.delta.x < 0 ? 1 : -1);
                        else
                            switchTab(lastEvent.delta.x < 0 ? 1 : -1);
            ...
            void dragndrop()
            {
                ...
                if (!(EditorWindow.mouseOverWindow is EditorWindow hoveredWindow)) return;
                if (!hoveredWindow.position.SetPos(0, 0).SetHeight(hoveredWindow.GetType() == t_SceneHierarchyWindow ? 20 : 40).Contains(lastEvent.mousePosition)) return;
        ...
        class TabInfo
        {
            public EditorWindow CreateWindow(object dockArea = null, bool atOriginalTabIndex = true)
            {
                ...
                void setupPropertyEditor()
                {
                    ...
                    if (!(globalId.GetObject() is Object obj)) return;


                    window.GetMemberValue("tracker").InvokeMethod("SetObjectsLockedByThisTracker", (new List<Object> { obj }));

                    window.SetMemberValue("m_GlobalObjectId", globalId.ToString());
                    window.SetMemberValue("m_InspectedObject", obj);
            ...
            public TabInfo(Object lockTo)
            {
                ...
                var lockToFolder = lockTo is DefaultAsset;

                typeName = lockToFolder ? t_ProjectBrowser.Name : t_PropertyEditor.Name;
